...
router.all("/", (req, res) => {
  res.send("Chain Reaction is online.");
});
...
export function setUpChainReact(io) {
  io.of("/chainreact").on("connection", (socket) => {
    ...
    socket.on("makeMove", ({ gameId, row, col }) => {
      ...
      const chainReactionOccurred = handleChainReaction(
        game,
        row,
        col,
        player.id
      ...
      game.currentTurn =
        game.players[(currentIndex + 1) % game.players.length].id;
